	...
	while {true} do {
		uisleep 5;
		{
			...
			if (!_disable && !(_x in zbe_cachedGroups)) then {
					...
					if (!isDedicated) then { [zbe_aiCacheDist, _x, zbe_minFrameRate, zbe_debug] execFSM "zbe_cache\zbe_aiCachingClient.fsm";
						} else  { [zbe_aiCacheDist, _x, zbe_minFrameRate, zbe_debug] execFSM "zbe_cache\zbe_aiCachingDedicated.fsm";
						};
				};
		} forEach allGroups;
	};
